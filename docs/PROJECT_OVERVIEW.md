...
| OpenRouter adapter                | `transcribe_client/openrouter.py`                   | `_transcribe_bytes` (retry), `transcribe` (chunking). Retry on 502/503/504; 429 caught via `raise_for_status()`→`HTTPError`. |
...
| `worker`           | `bot-v2-worker`         | `Dockerfile` (`cyberkitty119/worker:fix-vpnstack`) | — | `postgres` (healthy) | Entrypoint `entrypoint-worker.sh`, runs `workers/transcribe_worker.py`. `DISABLE_LOCAL_WHISPER=1`, `REMOTE_DEEPINFRA_SSH_ALIAS`/`RELAY_URL` for proxied DeepInfra. Bind-mounts `~/.ssh` ro. |
| `core-api`         | `bot-v2-core-api`       | `Dockerfile.api`               | `8001:8000`| `postgres` (healthy) | REST backend for miniapp and internal consumers. |
