...
| Path | Status | Problem | Planned Action | Priority |
|------|--------|---------|----------------|----------|
| `docker-compose.yml` (`bot`, `worker`, `api`, `telegram-bot-api`, `postgres`) | core | Несколько compose-файлов в корне, есть следы старых флагов (DeepInfra и др.). | Зафиксировать один «эталонный» prod-compose, остальные явно пометить dev/legacy или удалить. | M |
| `transkribator_modules/bot/handlers.py` | core | Смешаны старые/новые ветки, много флагов (`AGENT_FIRST`, beta и т.п.); 2262 строки. Пересечение video/audio-флоу. | Разделить на модули по потокам (`video.py`, `audio.py`, `text.py`, `agent.py`), вычистить неиспользуемые/устаревшие обработчики. | H |
...
| `api_server.py` | core | Возможны мёртвые эндпоинты, пересечение логики с ботом. | Составить список эндпоинтов и их реальное использование, выделить чистый API для miniapp. | M |
...
| `transcribe_segment_with_openrouter_gemini` (`transcriber_v4.py`) | core | Логика ретраев и логирования есть, но нет агрегированного мониторинга по ошибкам; 429 retry без backoff на line ~1953. | Собрать метрики по статусам (успешно/timeout/HTTP ошибки), добавить короткий отчёт в логи/метрики; добавить `asyncio.sleep(min(2 ** attempt, 30))` перед retry-continue при 429. | M |
| `transcribe_client/` (адаптеры: openrouter, deepinfra, gpu, local, di_worker, stub) | core | `TranscribeClient` выбирает ОДИН адаптер через `_resolve_default_adapter` — fallback между адаптерами при ошибке НЕ реализован. `openrouter.py`: `max_retries=3` hardcoded, 429 попадает в общий `except RequestException` неявно; дублирующий мёртвый код chunked transcription (строки 256–314). | Реализовать fallback на DeepInfra при OpenRouter failure (в `default_transcribe_media`); вынести `max_retries` в env `OPENROUTER_MAX_RETRIES` (default 6); явный 429-retry с экспоненциальным backoff + jitter и `Retry-After`; удалить мёртвый дублирующий блок. | H |
...
| `processing_jobs` | core/mixed | По пользователю 274232565 за 2026-01-06 нет джоб — часть логики может обходиться без очереди. `job.error` хранит сырой traceback (утеччка в API/miniapp/MAX-бот). | Проверить, какие `job_type` реально создаются/обрабатываются, отключить/удалить неиспользуемые; хранить в `job.error` краткое сообщение, а не traceback (см. RESEARCH.md AC2.1–AC2.2). | M |
...
| Path | Status | Problem | Planned Action | Priority |
|------|--------|---------|----------------|----------|
| `transkribator_modules/main.py` | core | Нет вызова `application.add_error_handler(...)` — `AttributeError: 'NoneType' object has no attribute 'from_user'` при update без message/callback_query (edited_message, channel_post, пустой update при `allowed_updates=Update.ALL_TYPES`). | Зарегистрировать `_error_handler` (логирует + send_message при наличии effective_chat); добавить early-return guard в `handle_message` если `update.effective_user is None` или `update.message is None` (RESEARCH.md AC3.1–AC3.3). | H |
...
| `transkribator_modules/bot/handlers.py:1358,1360` | core | `await _safe_edit_message(status_msg, str(exc))` — утечка `str(exc)` для GDrive/Dropbox ошибок скачивания. | Заменить `str(exc)` на дружелюбное сообщение для download-ошибок (RESEARCH.md AC2.3). | M |
