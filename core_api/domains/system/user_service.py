...
class ProfileService:
    ...
    def get_profile_by_telegram_id(self, telegram_id: int, tg_first_name: str = "", tg_last_name: str = "") -> UserProfileResponse:
        ...
        # Referral info
        from transkribator_modules.services.referral_service import ReferralService
        ref_service = ReferralService(self.db)
        ref_link_code = ref_service.get_referral_link(user)
        stats = ref_service.get_stats(user)
        referrals_count = stats.get('referrals_count', 0)
        reward_balance = stats.get('reward_balance', 0)

        # Google Drive connection status
        from transkribator_modules.services.google_service import GoogleCredentialService
        ...
        return UserProfileResponse(
            ...
            minutes_used_this_month=float(usage_info['minutes_used_this_month']),
            minutes_limit=float(usage_info['minutes_limit']),
            minutes_remaining=float(usage_info['minutes_remaining']),
            generations_used_this_month=int(usage_info['generations_used_this_month']),
            generations_limit=int(usage_info['generations_limit']),
            generations_remaining=int(usage_info['generations_remaining']),
            usage_percentage=float(usage_info['usage_percentage']),
            total_minutes_transcribed=float(usage_info['total_minutes_transcribed']),
