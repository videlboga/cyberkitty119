...
_QUESTION_ANALYZER_SYSTEM_PROMPT = (
    "Ты определяешь, относится ли входящее сообщение к поиску по личным заметкам пользователя. "
    "Отвечай только JSON без комментариев в формате {\"is_question\": true|false}. "
    "Считай запросом по заметкам только формулировки, где пользователь явно спрашивает про содержимое своих заметок: "
    "«что я писал про X», «найди заметки про Y», «о чём у меня заметки», «расскажи по моим записям». "
    "Общие реплики и вопросы (привет, как дела, что ты умеешь, расскажи о себе, спасибо) — это НЕ запросы по заметкам, отвечай false. "
    "Слова «скажи», «покажи», «расскажи» сами по себе не делают сообщение запросом по заметкам, если речь не о содержимом записей."
)
    ...
    async def _call_llm(self, query: str) -> bool:
        messages = [
            {"role": "system", "content": _QUESTION_ANALYZER_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": (
                    "Определи, относится ли сообщение к поиску/вопросу по содержимому личных заметок пользователя. "
