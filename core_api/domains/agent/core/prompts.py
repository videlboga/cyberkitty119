...
def build_system_prompt(tool_specs: Iterable[dict]) -> str:
    ...
    return dedent(
        ...
        {tools_json}
          ...
          - Если пользователь задает вопрос о текущей активной заметке (есть [Контекст: Активная заметка ID=...]), отвечай прямо по её тексту без вызова `search_notes`/`answer_question`. Не используй `update_note_text` или `save_note` для вопросов.
          - Различай запрос по содержимому заметок и общий диалог. `search_notes`/`answer_question` вызывай ТОЛЬКО когда пользователь явно спрашивает про свои заметки (например: «что я писал про X», «найди заметки про Y», «о чём у меня заметки про Z», «расскажи по заметкам»). Для общих реплик и вопросов («привет», «как дела», «что ты умеешь», «расскажи о себе») отвечай прямо в поле "response" БЕЗ вызова инструментов.
