            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.post(url, headers=headers, data=form_data) as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.warning(f"Временная ошибка DeepInfra ({response.status}) для {file_name}: {error_text}. Повтор через 2с")
                        await asyncio.sleep(2)
                        async with session.post(url, headers=headers, data=form_data) as resp2:
                            if resp2.status == 200:
                                data2 = await resp2.json()
                                return data2.get('text', '')
                            else:
                                err2 = await resp2.text()
                                logger.error(f"Повтор тоже неудачен ({resp2.status}) для {file_name}: {err2}")
                                return None
                    else:
                        error_text = await response.text()
