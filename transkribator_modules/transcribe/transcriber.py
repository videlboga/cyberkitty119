...
Саммари должно быть содержательным, информативным, но при этом структурированным и понятным."""
            ...
            if result:
                ...
                return "Мяу... Что-то не так с моими киберлапками! 🐾 Не получается создать подробное саммари. *грустно опускает ушки* Обратитесь к моему создателю @Like\\_a\\_duck - он точно поможет! 🚀"
        
        return "Мяу... У меня нет доступа к умным помощникам для создания саммари! 😿 Напишите @Like\\_a\\_duck - он настроит всё как надо! ⚙️"
            
    except Exception as e:
        logger.error(f"Ошибка при создании подробного саммари: {e}")
        return f"Ой-ой! Произошла киберошибка при создании подробного саммари! 🤖💥 Расскажите @Like\\_a\\_duck что случилось - он всё исправит! 🛠️\n\nТехническая информация: {str(e)}"
...
Саммари должно быть максимально коротким, но при этом информативным."""
            ...
            if result:
                ...
                return "Мяу... Мои киберлапки сегодня не слушаются! 🐾 Не получается создать краткое саммари. *виновато мяукает* Сообщите @Like\\_a\\_duck - он всё наладит! 💫"
        
        return "Мяу... У меня нет доступа к умным помощникам для создания саммари! 😿 Пишите @Like\\_a\\_duck - он подключит нужные сервисы! 🔌"
            
    except Exception as e:
        logger.error(f"Ошибка при создании краткого саммари: {e}")
        return f"Ай-ай! Киберошибка в моих схемах! 🤖⚡ Краткое саммари не получилось создать. Расскажите @Like\\_a\\_duck - он разберётся! 🔧\n\nДетали ошибки: {str(e)}"
