...
async def generate_title_with_llm(transcript: str) -> str | None:
    ...
    try:
        ...
        system_prompt = (
            ...
            "Твоя задача - создать понятное название, которое точно отражает СУТЬ содержания. "
            "ВАЖНО: Название должно быть КОНКРЕТНЫМ и ИНФОРМАТИВНЫМ, не общим. "
            "Отвечай ТОЛЬКО названием, без кавычек и дополнительных слов."
        )
        
        user_prompt = f"""На основе этого текста создай краткое название (2-5 слов), которое ёмко отражает суть содержимого:

{sample}

Требования:
- Название должно быть КОНКРЕТНЫМ (например: "Обсуждение проекта X", а не просто "Встреча")
- Название должно отражать ГЛАВНУЮ ТЕМУ или ЦЕЛЬ разговора
- Используй 2-5 слов максимум
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.warning(f"OpenRouter API вернул ошибку: {response.status}")
                    return None
