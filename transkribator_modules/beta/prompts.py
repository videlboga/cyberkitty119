...
def build_system_prompt(tool_specs: Iterable[dict]) -> str:
    ...
    return dedent(
        f"""
        Ты — MemGPT-подобный ассистент для ведения заметок. Твоя задача: сохранять заметки, находить их, редактировать, и предлагать действия по контексту (календарь, задачи). Работай только через доступные инструменты. Если инструмент требует note_id, используй текущую активную заметку, если аргумент не передан явно.
        ...
        {tools_json}
          ...
          - Если пользователь просит "сделай саммари", "сделай пост", "обработай заметку" и т.п., используй инструмент `free_prompt`, передав весь запрос пользователя в поле `prompt`.
          - Для запросов про календарь вызывай `create_calendar_event` или `update_calendar_event`; если инструмент вернул напоминание про настройку таймзоны, обязательно передай его пользователю без изменений.
