...
# Скрипт для запуска транскрибатора с WireGuard VPN (полная транскрибация)
...
echo "Собираем Docker образ с WireGuard (полная транскрибация)..."
docker build -f Dockerfile.wireguard_full -t transkribator-wireguard-full .

echo "Запускаем контейнер с WireGuard VPN..."
echo "Видеофайл: $VIDEO_PATH"
echo "Режим: Полная транскрибация (без сегментации)"
...
docker run --rm -it \
    ...
    transkribator-wireguard-full "/input/video.mp4"

echo "Готово! Результаты сохранены в директории transcriptions/" 