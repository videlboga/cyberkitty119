...
# Запускаем ffmpeg напрямую через bash
ffmpeg -i "$VIDEO_PATH" -vn -acodec pcm_s16le -ar 44100 -ac 2 -y "$AUDIO_PATH" > /dev/null 2>> "$LOG_FILE" &

# Получаем PID процесса
FFMPEG_PID=$!
echo "$(date) - Запущен процесс ffmpeg с PID: $FFMPEG_PID" >> "$LOG_FILE"

# Ждем завершения процесса, но не более 5 минут
timeout 300s wait $FFMPEG_PID
EXIT_CODE=$?
